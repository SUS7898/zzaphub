...
<div class="posts-view-container">
    ...
    <div class="posts-header-area">
        <h1 class="posts-title">${posts.title}
            <span style="color: #57606a; font-weight: 300;">#${posts.id}</span>
        </h1>
        <div class="posts-meta">
            <span class="btn-gh" style="background: #2da44e; color: #fff; border: none; border-radius: 2em; cursor: default;">Open</span>

            <strong> 
                <c:if test="${not empty posts.equippedTitle}">
                    <span class="user-title-badge">${posts.equippedTitle}</span>
                </c:if> ${posts.loginId}
            </strong>님이 작성 · 조회수 ${posts.viewCount}
    ...
    <div class="posts-bubble">
        <div class="posts-bubble-header">
            <strong>${posts.loginId}</strong> commented on ${posts.createdAt}
        </div>
        ...
        <c:forEach var="comment" items="${comments}">
            <div class="comment-wrapper ${comment.parentId != null ? 'reply-item' : ''}">
                <div class="posts-bubble">
                    <div class="posts-bubble-header">
                        <strong> 
                            <c:if test="${not empty comment.equippedTitle}">
                                <span class="user-title-badge">${comment.equippedTitle}</span>
                            </c:if> ${comment.loginId}
                        </strong>
                        <div style="display: flex; gap: 8px;">
                            ...
                            <c:if test="${sessionScope.role eq 'ADMIN' or sessionScope.role eq 'MANAGER' or sessionScope.id eq comment.loginId}">
                                <button class="btn-gh" style="color: #cf222e;"
                                    onclick="if(confirm('댓글을 삭제하시겠습니까?')) location.href='commentDelete?id=${comment.id}&postId=${posts.id}'">Delete</button>
                            </c:if>
                        </div>
                    </div>
                    <div class="posts-bubble-body">
                ...
                <div id="reply-form-${comment.id}" style="display: none; margin: 0 0 20px 45px;">
                    <form action="commentProc" method="post">
                        ...
                        <div style="text-align: right;">
                            <button type="submit" class="btn-gh btn-comment">Reply</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <c:if test="${not empty sessionScope.id}">
        <div class="posts-bubble" style="margin-top: 40px;">
            ...
            <form action="commentProc" method="post" style="padding: 16px;">
                ...
                <div style="text-align: right;">
                    <button type="submit" class="btn-gh btn-comment">Comment</button>
                </div>
            </form>
        </div>
    </c:if>
